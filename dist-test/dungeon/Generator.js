...
(function (TileType) {
    ...
    TileType[TileType["Stairs"] = 4] = "Stairs";
})(TileType || (TileType = {}));
...
export class Generator {
    ...
    generate() {
        ...
        if (rooms.length < 2) {
            ...
            const r1 = { id: 0, x: 2, y: 2, width: 6, height: 6, centerX: 5, centerY: 5 };
            const r2 = { id: 1, x: W - 9, y: H - 9, width: 6, height: 6, centerX: W - 6, centerY: H - 6 };
            rooms.push(r1, r2);
    ...
    createRoomsInLeaves(node, cells, rooms) {
        if (node.isLeaf()) {
            ...
            if (node.leftChild)
                this.createRoomsInLeaves(node.leftChild, cells, rooms);
            if (node.rightChild)
                this.createRoomsInLeaves(node.rightChild, cells, rooms);
        }
    }
    ...
    carveLCorridor(cells, x1, y1, x2, y2) {
        const W = this.options.width;
        const H = this.options.height;
    ...
    ensureReachability(cells, rooms, spawnPos, stairsPos) {
        const W = this.options.width;
        const H = this.options.height;
        const isVisited = (start) => {
                    ...
                    for (let dy = -1; dy <= 1; dy++) {
                        for (let dx = -1; dx <= 1; dx++) {
                            ...
                            if (nx >= 0 && nx < W && ny >= 0 && ny < H) {
                                ...
                                if (neighborType === TileType.Floor ||
