...
async function runPhase6E2EHarness() {
  ...
  try {
    ...
    audio.playSkillSFX(Vector3.Zero(), -12, 350);
    audio.playSwingSFX();
    audio.playGoldPickupSFX();
    audio.playGlobePickupSFX();
    audio.playItemPickupSFX();
    logPass("Synthetic SFX methods (Hit, Skill, Swing, Gold, Globe, Item) executed cleanly");
  } catch (err) {
  ...
  const testSaveKey = SaveManager.getSaveKey("slot_1");
  const dummyState: GameSaveStateV1 = {
    ...
    inventory: {
      ...
      maxWeight: 30,
      items: [
        { id: "item_sword_1", templateId: "sword_1", name: "Iron Sword", category: ItemCategory.Equipment, rarity: "magic" as any, weight: 2 },
      ],
      equipment: {
        head: null,
        chest: null,
        legs: null,
        mainHand: { id: "item_staff_1", templateId: "staff_1", name: "Arcane Staff", category: ItemCategory.Equipment, rarity: "rare" as any, weight: 2 },
        offHand: null,
      },
    },
    talents: {
  ...
  const legacyKey = SaveManager.getSaveKey("legacy_save_slot");
  ...
  // SaveManager state capture & restoration test
  const tempPlayer = new Player("test_player", scene);
  tempPlayer.level = 12;
  tempPlayer.gainXp(0);
  tempPlayer.inventory.gold = 500;
  tempPlayer.setArchetype("healer");

  const saveSuccess = SaveManager.save("manual_slot_1", tempPlayer, "town_hub", 1);
  if (saveSuccess && SaveManager.exists("manual_slot_1")) {
    logPass("SaveManager.save captured and persisted Player state to manual_slot_1");
  } else {
    logFail("SaveManager.save failed");
  }

  const metadata = SaveManager.getMetadata("manual_slot_1");
  if (metadata && metadata.level === 12 && metadata.archetype === "healer" && metadata.gold === 500) {
    logPass("SaveManager.getMetadata returned correct metadata (Lvl 12 HEALER, 500 Gold)");
  } else {
    logFail(`SaveManager.getMetadata mismatch: ${JSON.stringify(metadata)}`);
  }

  const restoredPlayer = new Player("restored_player", scene);
  const loadSuccess = SaveManager.load("manual_slot_1", restoredPlayer);
  if (loadSuccess && restoredPlayer.level === 12 && restoredPlayer.activeArchetypeId === "healer" && restoredPlayer.inventory.gold === 500) {
    logPass("SaveManager.load restored Player level 12, archetype 'healer', and 500 gold into restored Player entity");
  } else {
    logFail(`SaveManager.load restoration mismatch: level=${restoredPlayer.level}, arch=${restoredPlayer.activeArchetypeId}, gold=${restoredPlayer.inventory.gold}`);
  }
  ...
  // Resolve Critical Combat Damage (guarantee crit via 100% crit chance)
  player.stats.setBaseStat(StatType.CritChance, 1.0);
  DamageSystem.resolveDamage(player, enemy, 40, true);
  ...
  const testItem = { id: "item_gold_10", templateId: "gold_10", name: "Gold Coins", category: ItemCategory.Gold, goldAmount: 75, weight: 1 as const };
  const drop = new LootDrop("drop_gold", scene, testItem, player.position.clone().add(new Vector3(1, 0, 0)));

  // Simulate vacuum magnet pulling across multiple frame updates until picked up
  for (let tick = 0; tick < 20; tick++) {
    if (drop.isPickedUp) break;
  ...
  if (postHpObs === preHpObs) {
    logPass("Disposing HUD, InventoryUI, TalentUI, ArchetypeUI, and SaveLoadUI cleanly unsubscribes all registered observers from Player stats");
  } else {
